...
// ─── Constants ───
const CATEGORY_LABELS = { opkald:'Opkald', tilbud:'Tilbud', moede:'Moede', opfoelgning:'Opfoelgning', demo:'Demo', kontrakt:'Kontrakt', generelt:'Generelt' };
const PRIORITY_LABELS = { low:'Lav', normal:'Normal', high:'Hoej', urgent:'Hastende' };
const STATUS_LABELS = { open:'Aaben', in_progress:'I gang', done:'Faerdig' };
const LI_ACTIVITY_LABELS = { post:'Opslag', comment:'Kommentar', like:'Like', share:'Deling', article:'Artikel' };
const LI_ENGAGE_LABELS = { like:'Like', comment:'Kommentar', share:'Deling', follow:'Foelger' };
const TENDER_STATUS_LABELS = { draft:'Kladde', in_progress:'I gang', submitted:'Indsendt', won:'Vundet', lost:'Tabt' };
const TENDER_STATUS_COLORS = { draft:'bg-gray-100 text-gray-600', in_progress:'bg-blue-100 text-blue-700', submitted:'bg-yellow-100 text-yellow-700', won:'bg-green-100 text-green-700', lost:'bg-red-100 text-red-700' };
...
function sectorLabel(s) { return { el:'El', vand:'Vand', varme:'Varme', multiforsyning:'Multiforsyning' }[s] || s || 'Ukendt'; }
...
function renderTagBar(currentTags, allTags, entityType, entityId) {
    ...
    const input = h('input', {
        type: 'text', placeholder: 'Soeg eller opret tag...',
...
async function initUserSelector() {
    ...
    try {
        ...
        sel.innerHTML = '<option value="">Vaelg bruger...</option>';
...
async function renderDashboard(container) {
    container.innerHTML = '<div class="text-gray-400">Indlaeser dashboard...</div>';
    ...
    const statData = [
        { label:'Virksomheder', value:stats.total, color:'text-gray-900' },
        { label:'Staerke', value:stats.strong, color:'text-green-600' },
        { label:'Gode', value:stats.good, color:'text-yellow-600' },
        { label:'Svage', value:stats.weak, color:'text-orange-600' },
        { label:'Kolde', value:stats.cold, color:'text-red-600' },
    ];
    for (const s of statData) {
        statCards.appendChild(h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-5' },
    ...
    // Recent Activity Feed (øverst under stats)
    const recentActivities = dashData.recent_activities || [];
    if (recentActivities.length > 0) {
        container.appendChild(renderActivityFeed(recentActivities));
    }

    // Average score
    if (stats.total > 0) {
        container.appendChild(h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-8 flex items-center gap-4' },
            h('div', {},
            ...
            h('div', { className: 'flex-1' },
                h('div', { className: 'score-bar' },
                    h('div', { className: 'score-bar-fill', style: `width:${stats.avg_score}%;background-color:${stats.avg_score >= 80 ? '#059669' : stats.avg_score >= 50 ? '#d97706' : stats.avg_score >= 20 ? '#ea580c' : '#dc2626'}` })
                )
            )
        ));
    }
    ...
    filterBar.appendChild(h('span', { className: 'text-sm text-gray-500' }, 'Sektor:'));
    for (const s of ['Alle', 'el', 'vand', 'varme', 'multiforsyning']) {
        filterBar.appendChild(h('button', {
            className: `px-3 py-1 rounded-full text-sm font-medium ${s === 'Alle' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`,
            'data-filter': 'sector',
            'data-value': s === 'Alle' ? '' : s,
            onClick: (e) => applyDashboardFilter(e, scores, tableContainer)
        }, s === 'Alle' ? 'Alle' : sectorLabel(s)));
    }
    filterBar.appendChild(h('span', { className: 'text-sm text-gray-500 ml-4' }, 'Rating:'));
    for (const r of ['Alle', 'A', 'B', 'C']) {
        filterBar.appendChild(h('button', {
            className: `px-3 py-1 rounded-full text-sm font-medium ${r === 'Alle' ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`,
            'data-filter': 'rating',
            'data-value': r === 'Alle' ? '' : r,
            onClick: (e) => applyDashboardFilter(e, scores, tableContainer)
        }, r === 'Alle' ? 'Alle' : `${r}-kunde`));
    ...
    // Tag filter
    if (allTags.length > 0) {
        filterBar.appendChild(h('span', { className: 'text-sm text-gray-500 ml-4' }, 'Tag:'));
        filterBar.appendChild(h('button', {
            className: 'px-3 py-1 rounded-full text-sm font-medium bg-gray-700 text-white',
        ...
        for (const tag of allTags) {
            filterBar.appendChild(h('button', {
                ...
                'data-filter': 'tag', 'data-value': String(tag.id),
                onClick: (e) => applyDashboardFilter(e, scores, tableContainer)
            }, tag.name));
        }
    }
    ...
    renderScoreTable(tableContainer, scores);
}

function renderActivityFeed(activities) {
    const section = h('div', { className: 'mb-8' });

    section.appendChild(h('div', { className: 'flex items-center gap-3 mb-4' },
        h('div', { className: 'text-xl' }, '\u{1F4CB}'),
        h('h2', { className: 'text-lg font-bold text-gray-900' }, 'Seneste 14 dages aktivitet'),
        h('span', { className: 'text-sm text-gray-400' }, `${activities.length} aktiviteter`)
    ));

    const ACTIVITY_ICONS = {
        email: '\u{1F4E7}', phone: '\u{1F4DE}', meeting: '\u{1F91D}', linkedin: '\u{1F4BC}',
        post: '\u{1F4DD}', comment: '\u{1F4AC}', like: '\u{1F44D}', share: '\u{1F501}', article: '\u{1F4F0}', follow: '\u{1F514}',
        opkald: '\u{1F4DE}', tilbud: '\u{1F4B0}', moede: '\u{1F91D}', opfoelgning: '\u{1F504}', demo: '\u{1F4BB}', kontrakt: '\u{1F4C4}', generelt: '\u{2705}'
    };
    const ACTIVITY_LABELS = {
        email: 'Email', phone: 'Opkald', meeting: 'M\u00f8de', linkedin: 'LinkedIn',
        post: 'Opslag', comment: 'Kommentar', like: 'Like', share: 'Deling', article: 'Artikel', follow: 'F\u00f8lger',
        opkald: 'Opkald', tilbud: 'Tilbud', moede: 'M\u00f8de', opfoelgning: 'Opf\u00f8lgning', demo: 'Demo', kontrakt: 'Kontrakt', generelt: 'Opgave'
    };
    const SOURCE_COLORS = {
    ...
    for (const [day, items] of Object.entries(grouped)) {
        ...
        for (const item of items) {
            ...
            const colorClass = SOURCE_COLORS[item.source] || 'bg-gray-100 text-gray-700';

            const sourceLabel = item.source === 'interaction' ? label
                : item.source === 'task' ? `Opgave: ${label}`
                : item.source === 'linkedin_activity' ? `LinkedIn: ${label}`
                : `Engagement: ${label}`;

            // Line 2: who → who: subject
            ...
            if (item.subject) {
                ...
                detailLine.appendChild(h('span', { className: 'text-gray-400' }, item.subject));
            }

            const actItem = h('div', {
                className: 'activity-item',
                onClick: () => { location.hash = `#/companies/${item.company_id}`; }
            },
                ...
                h('div', { className: 'flex-1 min-w-0' },
                    h('div', { className: 'flex items-center gap-2' },
                        ...
                        h('span', { className: `activity-type-badge ${colorClass}` }, sourceLabel)
                    ),
                    detailLine
                )
            );
            timeline.appendChild(actItem);
        }
    }
...
let dashboardFilters = { sector: '', rating: '', tag: '' };
...
function applyDashboardFilter(e, allScores, tableContainer) {
    ...
    btn.className = `px-3 py-1 rounded-full text-sm font-medium ${filterType === 'sector' ? 'bg-gray-900' : 'bg-gray-700'} text-white`;
...
function renderScoreTable(container, scores) {
    ...
    const headerRow = h('tr', { className: 'bg-gray-50 border-b text-xs font-medium text-gray-500 uppercase tracking-wider' },
        ...
        h('th', { className: 'text-center px-2 py-3 w-12 cursor-pointer hover:text-gray-700 select-none', onClick: hdrClick('rating') }, 'Rtg' + si('rating')),
    ...
    for (const s of sorted) {
        ...
        tr.innerHTML = `
            ...
            <td class="text-center px-2 py-3"><span class="badge badge-${s.rating || 'C'}">${s.rating || 'C'}</span></td>
            <td class="px-2 py-3">
...
async function renderCompanies(container) {
    container.innerHTML = '<div class="text-gray-400">Indlaeser...</div>';
    ...
    for (const c of companies) {
        grid.appendChild(h('a', {
            ...
            h('div', { className: 'flex justify-between items-start mb-2' },
                h('h3', { className: 'font-semibold text-gray-900' }, c.name),
                h('div', { className: 'flex gap-1' },
                    c.rating ? h('span', { className: `badge badge-${c.rating}` }, c.rating) : null,
                    c.sector ? h('span', { className: `badge badge-${c.sector}` }, sectorLabel(c.sector)) : null
                )
            ),
    ...
    const form = h('form', { className: 'space-y-4', onSubmit: async (e) => {
        ...
        h('div', { className: 'grid grid-cols-2 gap-3' },
            h('div', {},
                ...
                h('select', { name: 'sector', className: 'w-full border border-gray-300 rounded-lg px-3 py-2' },
                    h('option', { value: '' }, 'Vaelg sektor...'),
                    ...['el', 'vand', 'varme', 'multiforsyning'].map(s =>
        ...
        h('div', {},
            ...
            h('select', { name: 'account_manager_id', className: 'w-full border border-gray-300 rounded-lg px-3 py-2' },
                h('option', { value: '' }, 'Vaelg ansvarlig...'),
...
async function renderCompanyDetail(container, id) {
    ...
    container.appendChild(h('div', { className: 'flex items-center gap-4 mb-6' },
        ...
        h('div', { className: 'flex-1' },
            h('div', { className: 'flex items-center gap-3' },
                ...
                company.rating ? h('span', { className: `badge badge-${company.rating} text-base` }, `${company.rating}-kunde`) : null,
                company.sector ? h('span', { className: `badge badge-${company.sector}` }, sectorLabel(company.sector)) : null
            ),
            h('div', { className: 'flex items-center gap-4 mt-1' },
    ...
    // Score card
    container.appendChild(h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6' },
        h('div', { className: 'flex items-center justify-between mb-4' },
            h('h2', { className: 'text-lg font-semibold text-gray-900' }, 'Relations-Score'),
            h('span', { className: `badge ${scoreBg(score.level)} text-base px-4 py-1` }, `${Math.round(score.score)} / 100`)
        ),
        h('div', { className: 'score-bar mb-4', style: 'height:12px' },
        ...
        h('div', { className: 'grid grid-cols-2 md:grid-cols-4 gap-4 text-sm' },
            scoreDetail('Interaktioner', `${score.interaction_points}/60`, `${score.total_interactions} i alt`),
            scoreDetail('Kontakt-daekning', `${score.coverage_points}/20`, `${score.contacted_count}/${score.total_contacts}`),
            scoreDetail('Kanal-mangfoldighed', `${score.diversity_points}/20`, score.channel_types.length > 0 ? score.channel_types.join(', ') : 'Ingen'),
            scoreDetail('Tidsforfald', `x${score.decay_factor}`, score.days_since_last != null ? `${score.days_since_last} dage` : 'Aldrig'),
        )
    ));
    ...
    if (openTasks.length === 0) {
        tasksSection.appendChild(h('p', { className: 'text-gray-400 text-sm' }, 'Ingen aabne sager.'));
    } else {
    ...
    const contactsSection = h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-6' },
        h('div', { className: 'flex justify-between items-center mb-4' },
            ...
            h('button', { className: 'text-blue-600 hover:text-blue-800 text-sm font-medium', onClick: () => showContactForm(id) }, '+ Tilfoej')
        )
    );
        ...
        for (const c of contacts) {
            ...
            contactsSection.appendChild(h('div', { className: 'flex items-center gap-3 py-3 border-b border-gray-100 last:border-0' },
                ...
                h('div', { className: 'flex gap-1 items-center' },
                    ...
                    c.on_linkedin_list ? h('span', { className: 'interaction-linkedin', innerHTML: interactionIcon('linkedin'), title: 'Paa LinkedIn liste' }) : null
                ),
                h('button', {
                    className: 'text-gray-300 hover:text-blue-600 ml-2', title: 'Registrer interaktion',
    ...
    const timelineSection = h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-6' },
        h('h2', { className: 'text-lg font-semibold text-gray-900 mb-4' }, `Interaktioner (${interactions.length})`)
    );
            ...
            timelineSection.appendChild(h('div', { className: 'flex gap-3 py-3 border-b border-gray-100 last:border-0' },
                ...
                h('div', { className: 'flex-1' },
                    h('div', { className: 'flex justify-between' },
                        h('span', { className: 'font-medium text-gray-900 text-sm' }, i.subject || i.type),
                        h('span', { className: 'text-xs text-gray-400' }, formatDate(i.date))
                    ),
                    h('div', { className: 'text-xs text-gray-500' }, `${i.contact_name || ''}${i.user_name ? ' \u2022 ' + i.user_name : ''}`),
...
function showContactForm(companyId, existing = null) {
    ...
    const form = h('form', { className: 'space-y-4', onSubmit: async (e) => {
        ...
        h('div', { className: 'flex justify-end gap-3 pt-2' },
            h('button', { type:'button', className:'px-4 py-2 text-gray-600 hover:text-gray-800', onClick: closeModal }, 'Annuller'),
            h('button', { type:'submit', className:'bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-medium' }, isEdit ? 'Gem' : 'Tilfoej kontakt')
        )
    );
    showModal(isEdit ? 'Rediger kontakt' : 'Tilfoej kontaktperson', form);
}
    ...
    const form = h('form', { className: 'space-y-4', onSubmit: async (e) => {
        ...
        h('div', {},
            ...
            h('div', { className: 'grid grid-cols-4 gap-2' },
                ...['email', 'phone', 'meeting', 'linkedin'].map(t =>
                    h('label', { className: 'flex flex-col items-center gap-1 p-3 border rounded-lg cursor-pointer hover:border-blue-400 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50' },
                        ...
                        h('span', { className: `interaction-${t}`, innerHTML: interactionIcon(t) }),
                        h('span', { className: 'text-xs' }, { email:'Email', phone:'Telefon', meeting:'Moede', linkedin:'LinkedIn' }[t])
                    )
                )
            )
        ),
        ...
        h('div', {},
            h('label', { className: 'block text-sm font-medium text-gray-700 mb-1' }, 'Saelger'),
            h('select', { name: 'user_id', className: 'w-full border border-gray-300 rounded-lg px-3 py-2' },
                h('option', { value: '' }, 'Vaelg saelger...'),
...
async function renderTenderDetail(container, id) {
    container.innerHTML = '<div class="text-gray-400">Indlaeser...</div>';
    const data = await api.getTenderFull(id);
    ...
    if (tender.description || tender.notes || tender.portal_link) {
        ...
        if (tender.portal_link) infoCard.appendChild(h('a', { href: tender.portal_link, target: '_blank', className: 'text-blue-600 hover:underline text-sm' }, 'Aaben udbudsportal'));
        if (tender.notes) infoCard.appendChild(h('p', { className: 'text-sm text-gray-500 mt-2' }, tender.notes));
        container.appendChild(infoCard);
    }

    // Gantt timeline
    if (sections.length > 0 && (tender.deadline || sections.some(s => s.deadline))) {
    ...
    if (sections.length === 0) {
        sectionList.appendChild(h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-400' },
            'Ingen sektioner. Tilfoej sektioner manuelt eller vaelg en skabelon ved oprettelse.'));
    }
...
function renderGanttTimeline(tender, sections, users) {
    ...
    // Extend end to include all section deadlines
    for (const s of sorted) {
        if (s.deadline) {
        ...
        for (const entry of auditEntries) {
            ...
            if (isContentEntry) {
                ...
                const headerRow = h('div', { className: 'flex items-center gap-2' },
                    ...
                    hasMore ? h('span', { className: 'section-tl-expand-hint' }, 'klik for at aabne') : null
                );
...
async function showTenderForm(users, existing = null) {
    const isEdit = !!existing;
    ...
    const form = h('form', { className: 'space-y-4', onSubmit: async (e) => {
        ...
        h('div', {},
            ...
            h('select', { name:'company_id', className:'w-full border border-gray-300 rounded-lg px-3 py-2', required:'' },
                h('option', { value:'' }, 'Vaelg...'),
                ...companies.map(c => h('option', Object.assign({ value: String(c.id) }, existing?.company_id == c.id ? { selected:'' } : {}), c.name))
            )
        ),
        !isEdit ? h('div', {},
        ...
        h('div', { className: 'grid grid-cols-2 gap-3' },
            formField('Estimeret vaerdi', 'estimated_value', existing?.estimated_value || ''),
...
function showSectionForm(tenderId, users, sectionCount, existing = null) {
    ...
    const form = h('form', { className: 'space-y-4', onSubmit: async (e) => {
        ...
        formField('Raekkefoelge', 'sort_order', String(existing?.sort_order ?? sectionCount), 'number'),
        h('div', { className: 'flex justify-end gap-3 pt-2' },
            h('button', { type:'button', className:'px-4 py-2 text-gray-600 hover:text-gray-800', onClick: closeModal }, 'Annuller'),
            h('button', { type:'submit', className:'bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-medium' }, isEdit ? 'Gem' : 'Tilfoej sektion')
        )
    );
...
async function renderTasks(container) {
    ...
    container.appendChild(h('div', { className: 'flex justify-between items-center mb-6' },
        h('div', {},
            ...
            h('p', { className: 'text-gray-500 mt-1' }, 'Opgaver og opfoelgninger')
        ),
        h('button', {
            className: 'bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium',
    ...
    const sumData = [
        ...
        { label:'Faerdige', value: summary.done, color:'text-green-600' },
    ...
    if (tasks.length === 0) {
        ...
        for (const status of ['open', 'in_progress', 'done']) {
            ...
            for (const t of group) {
                const isOverdue = t.due_date && t.due_date < new Date().toISOString().split('T')[0] && t.status !== 'done';
                taskList.appendChild(h('div', { className: `flex items-center gap-4 px-6 py-4 border-b border-gray-100 ${isOverdue ? 'task-overdue' : ''}` },
                    h('button', {
                    ...
                    h('div', { className: 'flex-1' },
                        ...
                        h('div', { className: 'flex flex-wrap gap-2 mt-1' },
                            ...
                            t.company_name ? h('a', { href: `#/companies/${t.company_id}`, className: 'text-xs text-blue-600 hover:underline' }, t.company_name) : null,
                            t.assigned_to_name ? h('span', { className: 'text-xs text-gray-500' }, t.assigned_to_name) : null,
                        )
                    ),
...
function createEmailDropzone(contacts, users, companyId) {
    ...
    const dropzone = h('div', { className: 'dropzone rounded-lg p-8 text-center cursor-pointer' },
        ...
        h('p', { className: 'text-gray-500 font-medium' }, 'Traek email-filer hertil (.eml)'),
        h('p', { className: 'text-gray-400 text-sm mt-1' }, 'Tip: Traek email fra Outlook til Skrivebordet foerst'),
    ...
    const selectsRow = h('div', { className: 'grid grid-cols-2 gap-3 mt-3' },
        h('div', {},
            ...
            h('select', { id:'dropzone-contact', className:'w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm' },
                h('option', { value:'' }, 'Vaelg kontakt...'),
                ...contacts.map(c => h('option', { value: c.id }, `${c.first_name} ${c.last_name}`))
            )
        ),
        h('div', {},
            h('label', { className: 'block text-xs font-medium text-gray-500 mb-1' }, 'Saelger'),
    ...
    async function handleFiles(files) {
        ...
        if (!contactId) {
            ...
            statusEl.textContent = 'Vaelg en kontaktperson foerst';
        ...
        if (uploaded === 0) {
            ...
            statusEl.textContent = 'Ingen email-filer fundet. Outlook tip: Traek emailen til Skrivebordet foerst, og traek .eml filen hertil.';
            statusEl.classList.remove('hidden');
            return;
        }
    ...
    async function handleDrop(e) {
        ...
        // 4. Nothing worked
        statusEl.className = 'mt-3 text-sm text-orange-500';
        statusEl.textContent = 'Kunne ikke laese email-data. Proev at traekke emailen direkte fra Outlook.';
        statusEl.classList.remove('hidden');
    }
...
async function renderUsers(container) {
    ...
    container.appendChild(h('div', { className: 'flex justify-between items-center mb-6' },
        h('div', {},
            h('h1', { className: 'text-2xl font-bold text-gray-900' }, 'Brugere / Saelgere'),
            h('p', { className: 'text-gray-500 mt-1' }, `${users.length} brugere`)
        ),
        h('button', {
            className: 'bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium',
    ...
    if (users.length === 0) {
        container.appendChild(h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center text-gray-400' }, 'Ingen brugere endnu.'));
        return;
    }
    const table = h('div', { className: 'bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden' });
    for (const u of users) {
...
async function renderSettings(container) {
    ...
    thresholdCard.appendChild(h('h2', { className: 'text-lg font-semibold text-gray-900 mb-4' }, 'Score Graensevaerdier'));
    thresholdCard.appendChild(h('p', { className: 'text-sm text-gray-500 mb-4' }, 'Bestemmer hvornaar en virksomhed faar en notifikation, hvis scoren falder under graensen for deres rating.'));
    ...
    const saveThresholdsBtn = h('button', {
        ...
        onClick: async () => {
            try {
                ...
                setTimeout(() => { saveThresholdsBtn.textContent = 'Gem graensevaerdier'; }, 2000);
            } catch (err) { alert(err.message); }
        }
    }, 'Gem graensevaerdier');
    ...
    decayCard.appendChild(h('p', { className: 'text-sm text-gray-500 mb-4' },
        'Konfigurer point-fald naar en virksomhed ikke har haft aktivitet i et bestemt antal dage. Reglerne er kumulative - den hoejeste matchende straf anvendes.'));
    ...
    function renderRulesTable() {
        ...
        if (rules.length === 0) {
            wrapper.appendChild(h('div', { className: 'text-sm text-gray-400 italic py-4' }, 'Ingen regler defineret. Tilfoej en regel for at komme i gang.'));
        } else {
            // Table header
    ...
    async function loadTemplates() {
        tmplListWrapper.innerHTML = '<div class="text-sm text-gray-400">Indlaeser...</div>';
        ...
        for (const tmpl of templates) {
            ...
            for (const sec of sections) {
                ...
                secRow.appendChild(h('div', { className: 'flex items-center gap-3' },
                    ...
                    h('span', { className: 'text-xs text-gray-400' }, `${sec.default_days_before_deadline}d foer deadline`)
                ));
    ...
    function showTemplateSectionForm(templateId, existing = null, onDone) {
        ...
        const form = h('form', { className: 'space-y-4', onSubmit: async (e) => {
            ...
            h('div', { className: 'grid grid-cols-2 gap-3' },
                formField('Dage foer deadline', 'default_days_before_deadline', String(existing?.default_days_before_deadline ?? 7), 'number'),
                formField('Raekkefoelge', 'sort_order', String(existing?.sort_order ?? 0), 'number')
            ),
            h('div', { className: 'flex justify-end gap-3 pt-2' },
                h('button', { type: 'button', className: 'px-4 py-2 text-gray-600', onClick: closeModal }, 'Annuller'),
                h('button', { type: 'submit', className: 'bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-medium' },
                    isEdit ? 'Gem' : 'Tilfoej sektion')
            )
        );
        showModal(isEdit ? 'Rediger sektion' : 'Ny sektion', form);
    }
